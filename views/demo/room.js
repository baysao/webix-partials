define(["app", "config", "ws"], function($app, $config, $ws) {
        ...
        rows: [
            {
                ...
                columns: [
                    ...
                    {id:"view_public", header:"&nbsp;", width:35, template:"<span  style=' cursor:pointer;' class='webix_icon fa-globe'></span>"},
                    {id:"view_secret", header:"&nbsp;", width:35, template:"<span  style=' cursor:pointer;' class='webix_icon fa-user-secret'></span>"},
		            {id:"edit", header:"&nbsp;", width:35, template:"<span  style=' cursor:pointer;' class='webix_icon fa-pencil'></span>"},
		            //{id:"sign_out", header:"&nbsp;", width:35, template:"<span  style='cursor:pointer;' class='webix_icon fa-sign-out'></span>"}
                ],
                pager: 'pagerA',
                onClick: {
                    // 'fa-sign-out': function(e, id){
                    //     console.log("id:" + id);
                    //     var args = {
                    //         cmd: 'room_members_delete',
                    //         uid: uid,
                    //         data: {id: $params.id, uid: uid}
                    //     }
                    //     $ws.sendText(ws, args)
                    ...
                    'fa-pencil': function(e, id){
                        console.log("id:" + id);
                        this.$scope.show('demo.room', {tab:'view_room_create',id:id, mode: 'edit'});
                    },
                    'fa-globe': function(e, id){
                        console.log("id:" + id);
                        this.$scope.show('demo.room', {tab:'view_room_create',id:id, mode: 'view_public'});
                    },
                    'fa-user-secret': function(e, id){
                        console.log("id:" + id);
                        this.$scope.show('demo.room', {tab:'view_room_create',id:id, mode: 'view_private'});
                    }
    ...
    var view_room_members = {
        ...
        rows: [
            ...
            {view: 'button', label: 'Save', id: 'btn_room_join_save'},
            {view: 'button', label: 'Sign-out', id: 'btn_room_join_signout'}
        ]
    ...
    return {
        $oninit: function($view, $scope) {
            ...
            $$('btn_room_join_signout').attachEvent("onItemClick", function(){
                var args = {
                    cmd: 'room_members_delete',
                    uid: uid,
                    data: {id: $params.id, uid: uid}
            ...
            $$('btn_room_join_save').attachEvent("onItemClick", function(){
                console.log("mem_left");console.log(members_left);
                console.log("mem_right");console.log(members_right);
                 var args = {
                     cmd: 'room_members_update',
                     uid: uid,
                     data: {id: $params.id, left: _.map(members_left, 'id'), right: _.map(members_right, 'id')}
                }
                //console.log($params);
                $ws.sendText(ws, args);
            })
        ...
        $onevent: {
            onMessage: function(e){
                ...
                case "room_read":
                    console.log("vao day");
                    console.log(data);
                    if(ctx == "view_room_create_view_public") {
                        if(_.isArray(data)) data = data[0];
                        $$('form_room_create').setValues(data, true);
                    } else if(ctx == "view_room_create_view_private") {
                        if(_.isArray(data)) data = data[0];
                        $$('form_room_create').setValues(data, true);
                    } else
                        $$('dt_room_list').clearAll();
                    $$('dt_room_list').parse(_.compact(data));
                    break;
                case "room_members_read":
                    console.log("vao day");
                    console.log(data);
                    members_left =  _.compact(data.left);
                    members_right = _.compact(data.right);
                    $$('room_members_left').clearAll();
                    $$('room_members_left').parse(members_left);
                    $$('room_members_right').clearAll();
                    $$('room_members_right').parse(members_right);
                    break;
            ...
            if(tab) {
                ...
                case "view_room_create":
                    ...
                    if(mode && room_id) {
                        switch(mode) {
                        case 'view_public':
                            $$('btn_room_create').hide();
                            var args = {cmd: "room_read", ctx: 'view_room_create_view_public', data: {id: room_id}};
                            $ws.sendText(ws, args);
                            break;
                        case 'view_private':
                            $$('btn_room_create').hide();
                            var args = {cmd: "room_read", ctx: 'view_room_create_view_private', uid: uid, room_token: $params.room_token, data: {id: room_id}};
                            $ws.sendText(ws, args);
                            break;
                        case 'edit':
                            $$('btn_room_create').show();
                            var args = {cmd: "room_read", ctx: 'view_room_create_view_private', uid: uid, room_token: $params.room_token, data: {id: room_id}};
                            $ws.sendText(ws, args);
                            break;
